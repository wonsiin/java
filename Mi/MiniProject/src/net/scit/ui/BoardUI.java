...
public class BoardUI {
	...
	private void read(UserVO vo) {
		...
		int c = b.getB_hitcount();
		
		System.out.print(">읽을 번호를 입력하세요 : ");
		b_num = sc.nextLine();
		BoardVO result = bdao.readBoard(b_num);

		b.setB_hitcount(ldao.getCount(result.getB_num()));
		...
		if(vo.getTeamnum().equals(result.getTeamnum())) {
			...
			System.out.print(result.getB_content());
			tuMenu();
			String im = sc.nextLine();
			switch (im) {
			case "1":	new BlikeUI(vo, b_num); break;

			case "2":	new ReplyUI(vo,b_num).readBoard(vo, b_num); break;

			case "3": new BoardUI(vo);
				break;
...
<<<<<<< HEAD
	public void tuMenu() {
		System.out.println();
		System.out.println("============[추천 & 덧글]===============");
		System.out.println("      1) 추천 하기");
		System.out.println("      2) 덧글 작성칸으로");
		System.out.println("      0) 게시판으로");
		System.out.println("==================================");
		System.out.print	("                선택 > ");
	}
=======
